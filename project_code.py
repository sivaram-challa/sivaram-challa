...
import numpy as np
import pandas as pd
df = pd.read_csv('C:/Users/chall/Downloads/aiml_project.csv')
...
def information_gain(left_child, right_child):
    ...
    if len(parent) > 0:
      ...
      p_parent = 0
    if len(left_child) > 0:
      p_left = left_child.count(1) / len(left_child)
    else:
      p_left = 0
    if len(right_child) > 0:
      p_right = right_child.count(1) / len(right_child)
    else :
      p_right = 0
...
model = random_forest(X_train, y_train, n_estimators=100, max_features=3, max_depth=20, min_samples_split=2)
print("PRIDICTION PROCESS ENDS")
